/**
 * CS 240 Shell Spells: starter code
 * Ben Wood, 2015
 *
 * Job list management
 *
...
typedef struct Job {
  ...
  /* Job ID of this job. */
  Jid jid;
  /* Process ID of this job's process. */
  pid_t pid;
...
/* Call function fp once on each job in jobs. (fp has two input parameters) */
void job_iter2(JobList* jobs, void (*fp)(JobList*,Job*));

/* Call function fp once on each job in jobs. (fp has three input parameters) */
void job_iter3(JobList* jobs, int choice, void (*fp)(JobList*, Job*, int choice));
